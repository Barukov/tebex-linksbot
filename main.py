...
def cmd_start(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        log.exception("fetch_packages failed")
        update.message.reply_text(f"Не смог получить товары из Tebex: {e}")
        return

    if not packages:
        update.message.reply_text("Не смог получить товары из Tebex.")
        return
...
def cmd_packages(update: Update, context: CallbackContext) -> None:
    user_id = update.effective_user.id
    if not is_admin(user_id):
        deny(update.message)
        return
...
def cmd_cancel(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text("Сбросил текущий сценарий.")
...
def on_callback(update: Update, context: CallbackContext) -> None:
    ...
    if not is_admin(user_id):
        query.message.reply_text("Нет доступа.")
        return
    ...
    if data == "refresh_packages":
        try:
            packages = fetch_packages()
            query.edit_message_text("Выбери товар.", reply_markup=package_keyboard(packages))
        except Exception as e:
            log.exception("refresh_packages failed")
            query.message.reply_text(f"Ошибка обновления: {e}")
        return
    ...
    if data.startswith("pkg:"):
        ...
        if not pkg:
            query.message.reply_text("Не нашёл этот товар. Нажми /start ещё раз.")
            return
...
def parse_usernames(text: str, expected_count: int):
    ...
    if len(usernames) != expected_count:
        return None, f"Ты отправил {len(usernames)} ников, а нужно либо 1 ник, либо ровно {expected_count} строк."
...
def on_message(update: Update, context: CallbackContext) -> None:
    ...
    if not package_id or not count:
        update.message.reply_text("Нажми /start и выбери товар с количеством.")
        return
    ...
    update.message.reply_text(f'Генерирую {count} ссылок...\nТовар: {package_name}\nЦена: {package_price}')
    ...
    for idx, username in enumerate(usernames, start=1):
        try:
            link = build_payment_link(package_id, username, user_id)
    ...
    update.message.reply_text(f"Готово. Успешно: {success}/{count}")
    USER_STATE.pop(user_id, None)
...
def main():
    ...
    dp.add_handler(CommandHandler("packages", cmd_packages))
